...
Визуализаторът (`lidar_map.py`) използва **Pygame** за рендиране в реално време.
Предишни опити с PyQtGraph/OpenGL имаха проблеми с производителността (замръзване,
препълване на буфери) и рендирането на линии създаваше артефакти тип „паяжина",
свързващи отдалечени точки през центъра.
...
|                                                       |
|  +-------------+     +-----------------------------+  |
|  | LidarSerial  |---->| LidarMap (Pygame рендерер)  |  |
|  |              |     |                             |  |
|  | - Сериен I/O |     | - 720-слотов скан буфер     |  |
|  | - Парсване   |     | - Мрежа с разстояния        |  |
|  |   на пакети  |     | - Детекция на стенни        |  |
|  | - Управление |     |   сегменти                  |  |
|  |   на буфера  |     | - Анимация на сканиращ лъч  |  |
|  +-------------+     | - HUD с жива статистика     |  |
|                       +-----------------------------+  |
+-------------------------------------------------------+
```
...
```
pyserial    - Комуникация по сериен порт
pygame      - 2D рендиране в реално време при 60 FPS
```

### Дизайн на скан буфера

Скан буферът използва масив от 720 слота (резолюция 0.5 градуса на слот), съхраняващ
последното измерване за всеки ъгъл:

```python
SCAN_SIZE = 720
scan_data = [None] * SCAN_SIZE

idx = int(angle * 2) % SCAN_SIZE  # 45.5 градуса -> слот 91
```
...
скана се отхвърлят, осигурявайки актуална визуализация на обкръжението.

### Детекция на стенни сегменти

Стените се рисуват чрез свързване на съседни точки, които са физически близки.
Алгоритъмът използва **разстояние в пикселно пространство** за определяне на
свързаност:
...
for j in range(1, len(screen_points)):
    ...
    if idx2 - idx1 > 10:  # ъглова разлика > 5 градуса -> прекъсване
        continue
...
Това предотвратява ефекта „паяжина", който се появяваше при свързване на точки
само по ъгъл — две точки с близки ъгли, но различни разстояния, създаваха линии
през центъра на дисплея.

### Клавишни команди

| Клавиш | Действие |
|--------|----------|
| `+` / `=` | Приближаване (намаляване на обхват) |
| `-` | Отдалечаване (увеличаване на обхват) |
| `W` | Вкл./Изкл. рендиране на стени |
| `G` | Вкл./Изкл. мрежа |
| `R` | Нулиране на скан данни |
| `F` | Вкл./Изкл. цял екран |
| `ESC` / `Q` | Изход |
